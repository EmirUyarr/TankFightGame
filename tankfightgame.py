...
def game_controls():
    ...
    while gcont:
        ...
        gameDisplay.fill(black)
        gameBackground = pygame.image.load("control.jpg")
        gameBackground = pygame.transform.scale(gameBackground, (display_width, display_height))
        gameDisplay.blit(gameBackground, (0, 0))
        message_to_screen("Controls", Rgb, -100, size="large")
        message_to_screen("Fire: Spacebar", Rgb, -30)
        message_to_screen("Move Turret: Up and Down arrows", Rgb, 10)
        message_to_screen("Move Tank: Left and Right arrows", Rgb, 50)
        message_to_screen("Press D to raise Power AND Press A to lower Power", Rgb, 90)
        message_to_screen("Pause: P", Rgb, 140)
        ...
        clock.tick(15)
def gameLoop():
    pygame.mixer.music.load("gamesound2.mp3")
    ...
    randomHeight = random.randrange(int(display_height * 0.1), int(display_height * 0.4))
    gameBackground = pygame.image.load("playbackground.jpeg")
    gameBackground = pygame.transform.scale(gameBackground, (display_width, display_height))
    ...
    while not gameExit:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    if mainTankX < display_width - tankWidth:
                        tankMove = 5
        ...
        if mainTankX - (tankWidth / 2) < 0:
            mainTankX = tankWidth / 2
        elif mainTankX + (tankWidth / 2) > display_width:
            mainTankX = display_width - tankWidth / 2

        if mainTankY - (tankHeight / 2) < 0:
            mainTankY = tankHeight / 2
        elif mainTankY + (tankHeight / 2) > display_height:
            mainTankY = display_height - tankHeight / 2

        if mainTankX - (tankWidth / 2) < xlocation + barrier_width:
            mainTankX += 5

        gameDisplay.fill(black)
        gameDisplay.blit(gameBackground, (0, 0))  # Arka plan resmini her çerçeve döngüsünde tekrar çiziyoruz.
        health_bars(player_health, enemy_health)
        gun = tank(mainTankX, mainTankY, currentTurPos)
        enemy_gun = enemy_tank(enemyTankX, enemyTankY, 8)
...
def fireShell(xy, tankx, tanky, turPos, gun_power, xlocation, barrier_width, randomHeight, enemyTankX, enemyTankY):
    ...
    while fire:
        ...
        if check_x_1 and check_x_2 and check_y_1 and check_y_2:
            ...
            explosion(hit_x, hit_y)
            fire = False
        pygame.mixer.Sound("explosion1.mp3").play() 
        pygame.display.update()
        clock.tick(60)
...
def game_over():
    ...
    while game_over:
        ...
        gameDisplay.fill(black)
        game_over_img = pygame.image.load("gameoverlost.jpg")
        game_over_img = pygame.transform.scale(game_over_img, (display_width, display_height))
        gameDisplay.blit(game_over_img, (0, 0))
...
def you_win():
    ...
    while win:
        ...
        gameDisplay.fill(black)
        game_over_img = pygame.image.load("gameoverwin.jpg")
        game_over_img = pygame.transform.scale(game_over_img, (display_width, display_height))
        gameDisplay.blit(game_over_img, (0, 0))
